...
func (ck *Clerk) Put(key string, value string) {
	ck.PushAppend(key, value, "Put")
}

func (ck *Clerk) Append(key string, value string) {
	ck.PushAppend(key, value, "Append")
}
...
//
func (ck *Clerk) PushAppend(key string, value string, op string) {
	ck.mu.Lock()
	defer ck.mu.Unlock()
	...
	for {
		...
		if ok {
			...
			for _, srv := range servers {
				...
				ok := common.Call(srv, "ShardKV.PushAppend", &args, &reply)
				if ok && reply.Err == OK {
					return
				}
				if ok && (reply.Err == ErrWrongGroup) {
